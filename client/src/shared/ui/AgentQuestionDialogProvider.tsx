...
import type { AgentQuestion } from '../types/ipc';
...
export function AgentQuestionDialogProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const agent = window.yibiao?.agent;
    if (!agent) return () => {};
    let active = true;
  ...
  const canSubmit = Boolean(question && selectedOption && (!selectedOption.custom || customAnswer.trim()));
  ...
  const submitAnswer = async () => {
    ...
    try {
      const agent = window.yibiao?.agent;
      if (!agent) throw new Error('Agent 提问通道不可用');
    ...
    <>
      ...
      <Dialog.Root open={Boolean(question)}>
        <Dialog.Portal>
          ...
          >
            <header className="agent-question-head"><Dialog.Title>需要您确认以下问题</Dialog.Title></header>
            <div className="agent-question-body">
              <div className="agent-question-copy"><MarkdownRenderer allowRawHtml={false} linkMode="text">{question?.question || ''}</MarkdownRenderer></div>
              <div className="agent-question-options" role="radiogroup" aria-label="Agent 提供的选项">
                {question?.options.map((option) => (
                  <label key={option.id} className={`agent-question-option${selectedOptionId === option.id ? ' is-selected' : ''}`}>
                    <input type="radio" name="agent-question-option" value={option.id} checked={selectedOptionId === option.id} disabled={submitting} onChange={() => setSelectedOptionId(option.id)} />
              ...
              {selectedOption?.custom && (
                <textarea className="agent-question-custom-answer" value={customAnswer} disabled={submitting} placeholder="请输入具体要求" aria-label={`${selectedOption.label}的具体要求`} autoFocus onChange={(event) => setCustomAnswer(event.target.value)} />
              )}
            </div>
            <footer className="agent-question-actions">
              <button type="button" className="primary-action" disabled={!canSubmit || submitting} onClick={() => void submitAnswer()}>
