...
function PiAgentMonitorPage() {
  ...
  const openMonitorWindow = async () => {
    try {
      await getBridge().developerAgentMonitor.openWindow();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Pi Agent 监视器打开失败');
    }
  };
        ...
        <section style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{ border: '1px solid #e2e8f0', borderRadius: 12, padding: 16, background: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button type="button" onClick={() => { void openMonitorWindow(); }} disabled={busy} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #cbd5e1', background: '#fff' }}>
                打开执行监视器
              </button>
...
export default PiAgentMonitorPage;
