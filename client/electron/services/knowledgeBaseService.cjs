...
const SEGMENT_BUDGET_META = { index: 999999, total: 999999 };

function now() {
  return new Date().toISOString();
}

function createKnowledgeTaskInterruptedError() {
  const error = new Error('知识库任务已暂停或取消');
  error.code = 'KNOWLEDGE_TASK_INTERRUPTED';
  return error;
}
...
function splitBusinessBlocksByPromptBudget(blocks, segmentLimit) {
  const numericLimit = Number(segmentLimit);
  ...
  const appendBlock = (block, content) => {
    result.push({
      ...block,
      id: `P${String(result.length + 1).padStart(6, '0')}`,
      content,
    });
  };
  ...
  for (const block of (Array.isArray(blocks) ? blocks : [])) {
    ...
    const blockWithFinalIdShape = { ...block, id: 'P000000', content };
    if (renderBlocksForPrompt([blockWithFinalIdShape]).length <= limit) {
      appendBlock(block, content);
      continue;
    }

    const shellChars = renderBlocksForPrompt([{ ...blockWithFinalIdShape, content: '' }]).length;
    const contentLimit = limit - shellChars;
    if (contentLimit < 1) {
      throw new Error(
        `知识库固定提示已占满当前请求预算，无法容纳业务 block ${block.id || '未知'}：可用 ${limit}，结构 ${shellChars}`,
      );
    }
    for (let offset = 0; offset < content.length; offset += contentLimit) {
      appendBlock(block, content.slice(offset, offset + contentLimit));
    }
  }
...
function filterBlocks(rawBlocks, segmentLimit = Infinity) {
...
/** 单个业务 block 不跨 Segment；超出本阶段可用预算时在任何 AI 请求前失败。 */
function packBlocksWithinBudget(blocks, segmentLimit, requestBudget, stage) {
  const source = Array.isArray(blocks) ? blocks : [];
  if (!source.length) return [];
  const limit = Math.max(0, Math.floor(Number(segmentLimit) || 0));
  for (const block of source) {
    const blockChars = renderBlocksForPrompt([block]).length;
    if (blockChars > limit) {
      throw new Error(
        `${stage} block ${block.id || '未知'} 超过当前可用预算：${blockChars}/${limit}（请求预算 ${requestBudget}）`,
      );
    }
  }
  return packBlocksIntoSegments(source, limit);
}
...
/** 按稳定输入顺序认领已有条目匹配，保证每个 block 最多归属一个条目 */
function mergeUniqueMatchResults(matchLists, itemIds, blocks, blockOrder) {
  return mergeRecoverySegmentResults(
    (matchLists || []).map((matches) => ({
...
function buildRecoveryTaskMessage(documentName, items) {
  return {
    ...
    content: [
      `文档名：${documentName}`,
      '你是投标知识库遗漏段落补漏助手。必须覆盖上文全部遗漏 block，每个 block 只能归入一类：',
      '1. matches：归入已有条目，返回已有 id 和 ranges。',
      '2. new_items：有复用价值但无合适条目时，返回 title、summary、ranges。',
      '3. discarded：低质、重复、格式残留或无复用价值时，返回 ranges 和 reason。',
      '只返回 JSON：{"matches":[{"id":"K000001","ranges":[["P000001","P000003"]]}],"new_items":[{"title":"","summary":"","ranges":[["P000004","P000005"]]}],"discarded":[{"ranges":[["P000006","P000006"]],"reason":""}]}',
      '仅使用本段 block 编号和本次条目 id；不要输出正文、Markdown 或解释。',
      '',
      '<knowledge_items>',
...
function assertMessagesWithinRequestBudget(messages, requestBudget, stage) {
  const requestChars = getMessagesContentLength(messages);
  if (requestChars > requestBudget) {
    throw new Error(`${stage}请求超过当前文本模型上下文预算：${requestChars}/${requestBudget}`);
  }
  return requestChars;
}

function getStageBlockBudget(requestBudget, fixedMessageLengths) {
  const minimumReserve = Math.max(1, Math.floor(requestBudget * TASK_AND_ITEMS_RESERVE_RATIO));
  const fixedMessageChars = Math.max(0, ...(fixedMessageLengths || []).map((value) => Math.floor(Number(value) || 0)));
  const reserve = fixedMessageChars + minimumReserve;
  const blockSegmentLimit = Math.max(0, requestBudget - reserve);
  return { blockSegmentLimit, reserve, minimumReserve, fixedMessageChars };
}
 ...
 */
function buildUnifiedBlockSegments(blocks, aiService, documentName = '') {
  const requestBudget = getRequestBudget(aiService);
  const fixedMessageLengths = [
    getMessagesContentLength(buildInitialItemMessages(documentName, '', SEGMENT_BUDGET_META)),
    getMessagesContentLength(buildSupplementItemMessages(documentName, '', [], SEGMENT_BUDGET_META)),
    getMessagesContentLength(buildMatchMessages(documentName, '', [], SEGMENT_BUDGET_META)),
  ];
  const budget = getStageBlockBudget(requestBudget, fixedMessageLengths);
  const segments = packBlocksWithinBudget(
    blocks,
    budget.blockSegmentLimit,
    requestBudget,
    '知识库提取/补充/匹配',
  );
  return {
...
function buildRecoveryBlockSegments(blocks, aiService, documentName = '') {
  const requestBudget = getRequestBudget(aiService);
  const fixedMessageLength = getMessagesContentLength(
    buildRecoveryMessages(documentName, [], [], SEGMENT_BUDGET_META),
  );
  const budget = getStageBlockBudget(requestBudget, [fixedMessageLength]);
  return {
    segments: packBlocksWithinBudget(
      blocks,
      budget.blockSegmentLimit,
      requestBudget,
      '知识库遗漏补漏',
...
function getBusinessBlockBudget(aiService, documentName = '') {
  const unified = buildUnifiedBlockSegments([], aiService, documentName);
  const recovery = buildRecoveryBlockSegments([], aiService, documentName);
  const blockSegmentLimit = Math.min(unified.blockSegmentLimit, recovery.blockSegmentLimit);
  if (blockSegmentLimit < 1) {
    throw new Error(
      `知识库固定提示已占满当前文本模型请求预算：unified=${unified.blockSegmentLimit}，recovery=${recovery.blockSegmentLimit}，request=${unified.requestBudget}`,
    );
  }
  return {
    blockSegmentLimit,
    requestBudget: unified.requestBudget,
    unifiedBlockSegmentLimit: unified.blockSegmentLimit,
    recoveryBlockSegmentLimit: recovery.blockSegmentLimit,
  };
}
...
function createKnowledgeBaseService({ app, aiService, configStore, knowledgeBaseStore }) {
  ...
  async function prepareDocument(documentId, sourceFilePath, webContents) {
    ...
    try {
      ...
      const businessBlockBudget = getBusinessBlockBudget(aiService, document.file_name);
      const persistedBlocksFitBudget = blocks.length > 0 && blocks.every(
        (block) => renderBlocksForPrompt([block]).length <= businessBlockBudget.blockSegmentLimit,
      );
      if (stepCanReuse(blockStep, persistedBlocksFitBudget)) {
        ...
        const result = await runDocumentStep(documentId, 'build_blocks', async () => {
          ...
          debugLog(documentId, 'prepare:blocks-ready', {
            ...
            business_block_limit: businessBlockBudget.blockSegmentLimit,
      ...
      const unifiedPack = buildUnifiedBlockSegments(blocks, aiService, document.file_name);
      ...
      if (stepCanReuse(firstStep, Array.isArray(firstItems))) {
        ...
        const result = await runDocumentStep(documentId, 'extract_first_items', async () => {
          ...
          const runSegment = async (segment) => {
            ...
            const requestChars = assertMessagesWithinRequestBudget(
              firstMessages,
              unifiedPack.requestBudget,
              '知识库条目提取',
            );
          ...
          if (segments.length > 1) {
            ...
            await waitForPromptCacheWarmup(() => activePreparations.has(documentId));
          }
      ...
      if (stepCanReuse(supplementStep, Array.isArray(supplementItems))) {
        ...
        const result = await runDocumentStep(documentId, 'extract_supplement_items', async () => {
          ...
          let completedSegments = 0;
          const runSegment = async (segment) => {
            const fullMessages = buildSupplementItemMessages(document.file_name, segment.text, firstItems, segment);
            const blockPrefixMessage = fullMessages[0];
            const fullLength = getMessagesContentLength(fullMessages);
            let itemSegments = [{ items: firstItems, index: 1, total: 1 }];
            if (fullLength > unifiedPack.requestBudget) {
              const itemSegmentLimit = getItemSplitBudget(aiService, [
                blockPrefixMessage,
                buildSupplementItemTaskMessage(document.file_name, []),
              ]);
              itemSegments = packItemsIntoSegments(firstItems, itemSegmentLimit);
              if (!itemSegments.length) itemSegments = [{ items: [], index: 1, total: 1 }];
            ...
            for (const itemSegment of itemSegments) {
              const supplementMessages = buildSupplementItemMessages(
                document.file_name,
                segment.text,
                itemSegment.items,
                segment,
              ...
              debugLog(documentId, 'ai:supplement-items:start', {
                ...
                item_mode: itemSegments.length > 1 ? 'sub_batch' : 'full',
              ...
              const supplement = await aiService.collectJsonResponse({
                ...
                temperature: 0.2,
                response_format: { type: 'json_object' },
                logTitle: itemSegments.length > 1
                  ? `知识库条目补充-${document.file_name}-第${segment.index}段-条目${itemSegment.index}`
                  : segments.length > 1
              ...
              itemLists.push(Array.isArray(supplement?.items) ? supplement.items : []);
            }
            const items = mergeTitleSummaryItems(itemLists);
            completedSegments += 1;
            updateDocument(documentId, {
              status: 'extracting',
  ...
  async function matchDocument(documentId, webContents, options = {}) {
    ...
    try {
      ...
      for (const segment of blockSegments) {
        ...
        try {
          ...
          if (fullItemLength <= requestBudget) {
            ...
            segmentMatches = mergeUniqueMatchResults(
              [parsed.matches],
              candidateItemIds,
              segmentBlocks,
              segmentBlockOrder,
      ...
      const mergedMatches = mergeUniqueMatchResults(
        [matches],
        candidateItemIds,
        blocks,
        blockOrder,
      ...
      if (!force && recoveryStep?.status === 'success' && isRecoveryStepResult(recoveryResult)) {
        ...
        recoveryResult = await runDocumentStep(documentId, 'recover_missing', async () => {
          ...
          for (let attempt = 0; attempt < recoveryMaxAttempts; attempt += 1) {
            ...
            // missing 在前：按真实固定消息扣预算；条目过长时仅拆 L2 条目，L1 保持不变
            const recoveryPack = buildRecoveryBlockSegments(missingBlocks, aiService, document.file_name);
            const recoveryRequestBudget = recoveryPack.requestBudget;
            const missingSegmentLimit = recoveryPack.blockSegmentLimit;
            const missingSegments = recoveryPack.segments;
            ...
            const runMissingSegment = async (missingSegment) => {
              ...
              if (fullLength <= recoveryRequestBudget) {
                ...
                segmentParsed = mergeRecoverySegmentResults(
                  [normalized],
                  currentItemIds,
                  segmentBlocks,
                  segmentBlockOrder,
            ...
            if (missingSegments.length > 1) {
              ...
              await waitForPromptCacheWarmup(() => activeMatches.has(documentId));
            }
...
module.exports = {
  ...
  _internals: {
    ...
    packBlocksWithinBudget,
    splitBusinessBlocksByPromptBudget,
    packItemsIntoSegments,
    getKnowledgeBaseSegmentLimit,
    buildUnifiedBlockSegments,
    buildRecoveryBlockSegments,
    getBusinessBlockBudget,
    getItemSplitBudget,
