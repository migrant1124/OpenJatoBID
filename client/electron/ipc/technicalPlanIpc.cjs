...
function registerTechnicalPlanIpc({ technicalPlanStore }) {
  ...
  ipcMain.handle('technical-plan:apply-outline-deepening', (_event, payload) => technicalPlanStore.applyOutlineDeepening(payload));
  ipcMain.handle('technical-plan:set-outline-deep-writing', (_event, payload) => technicalPlanStore.setOutlineDeepWriting(payload));
